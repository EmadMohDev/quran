...
function getEditionType($type)
{
    return EditionType::where('title', 'like', '%'.$type.'%')->first()->id;
}
...
function checkInUrl($keyword)
{
    $urlArray = explode('/', request()->route()->uri);

    if (in_array($keyword, $urlArray))
        return true;

    return false;
}

function transField($object)
{
    if (app()->getLocale() == 'ar')
        return $object->name;
...
function getPramOpId()
{
    return request()->has('opId') ? '?opId='.request()->opId : '';
}

function providerRedirect($provider, $type = 'quran')
{
    return $provider->editions()->count() > 1
    ? url('provider/'.$provider->id.'/editions')
    : url($type.'/'.$provider->editions()->first()->identifier.'/surahs');
}

function providerFeatures($key)
{
    $url = request()->segment(count(request()->segments()));
    return ($url == 'provider' || $url == $key) ? false : true;
}
