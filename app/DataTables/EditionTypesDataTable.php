...
class EditionTypesDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('check', function (EditionType $edition_type) {
                return '<input type="checkbox" class="check-box-id" value="'.$edition_type->id.'">';
            })
            ->addColumn('action', 'edition-types.action')
     ...
     */
    public function query(EditionType $model)
    {
    ...
    public function html()
    {
        return $this->builder()
                    ...
                    ->orderBy(1)
                    ->buttons([
                        Button::make()->text('<i class="fa fa-plus"></i>')->addClass('btn btn-info '.(false ? 'd-none' : ''))->action("window.location.href = " . '"' . url('edition-types/create') . '"')->titleAttr('Create New Edition Type (c)')->key((false ? '' : '')),
    ...
    {
        return [
            Column::make('check')->title('<input type="checkbox" id="check-all">')->exportable(false)->printable(false)->orderable(false)->searchable(false)->addClass('text-center')->width('2%'),
    ...
    {
        return 'EditionTypes_' . date('YmdHis');
    }
}
